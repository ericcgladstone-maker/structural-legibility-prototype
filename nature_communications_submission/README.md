...
This folder is the data and code reproducibility deposit accompanying the manuscript. The manuscript text, its rendered figures, its tables, and its Supplementary Information document live in the journal submission and are not duplicated here. The contents below let a third party regenerate the reported figures and the statistical-analysis values from the experimental data archived in this folder.

## Contents

### `data/` — raw experimental data

Twenty fixed event records, four primary-run JSON Lines streams with their derived feature and prediction streams, and the matching cross-reader-family slice. About 130 MB total. Each `*.jsonl` file is one record per line.

| File | Records | Description |
|---|---:|---|
| `worlds_v0_2.jsonl` | 20 | Structured event records that anchor every experimental cell. Each carries a core observed event, competing causal hypotheses with their supporting evidence, peripheral operational details, and a built-in conflict pair with proposition truth values, uncertainty levels, and evidence types. The records are synthetic, not empirical events. |
...
| `machine_tracing_bprime_v0_1.terminal_features.jsonl` | 4,200 | Fourteen surface features extracted per terminal message, plus the proposition-matched preservation rate `true_accuracy_score`. |
...
- `figure_style.py` — shared style module: color palettes, font configuration, Wilson 95% confidence interval helper, JSON Lines data loaders, motif vocabulary, and output-path helpers used by every figure script.
- `make_figure_1.py` through `make_figure_4.py` — generate the four main figures from the data in `data/`.
- `make_si_figure_1.py` through `make_si_figure_3.py` — generate the three supplementary figures from the same data.
- `bootstrap_and_entropy.py` — paired bootstrap 95% confidence intervals for the two accuracy gaps reported in the Supplementary Information, plus predicted-label Shannon entropy values for both reader instruments.

## How to run

The figure and bootstrap scripts read the JSON Lines streams from `data/` via the loaders in `figure_style.py`. With the deposit unpacked, each script can be executed independently from this `scripts/` folder. The figure scripts produce vector PDF and 600-dpi PNG outputs. The bootstrap script writes a single Markdown summary with paired-bootstrap intervals and entropy values, and prints a one-line summary to standard output.
...
Because language-model decoding is stochastic, the reproducibility standard for the upstream generation step is distributional, the same statistical patterns over multiple runs, rather than exact byte-equivalence of specific outputs. The analyses on the data deposited here are deterministic.
...
To be confirmed at publication. Code is intended under the MIT License and data under CC-BY-4.0.
